...
export class UsersDataSourceImpl implements UsersDataSource {
  ...
  async getUserById(payload: QueryUserRequestDto): Promise<UserResponseDto> {
    try {
      ...
      if (rawCall.status !== 200) {
        throw new Error(
          `Error fetching user with id ${payload.userId}: ${rawCall.statusText}`,
        );
      }
  ...
  async updateUser(
    payload: QueryUserRequestDto,
    userData: UpdateUserDto,
  ): Promise<UserResponseDto> {
    try {
      const rawCall = await this.api.patch(
        `/users/${payload.userId}`,
        userData,
      );
      if (rawCall.status !== 200) {
        throw new Error(
          `Error updating user with id ${payload.userId}: ${rawCall.statusText}`,
        );
      }
      const response: UserResponseDto = rawCall.data;
      return response;
